...
type mockConnectionInspector struct {
	network.ConnectionInspector
}

func (mockConnectionInspector *mockConnectionInspector) IsDomainReachable(domain string) bool {
...
func (mockConnectionInspector *mockConnectionInspector) GetPeerCertificates(address string, port uint16) ([]*x509.Certificate, error) {
	if port == 100 {
...
func TestUpdateCertificateInfo(t *testing.T) {
	...
	mockConnectionInspector := &mockConnectionInspector{}

	updater := &CertificateUpdater{mockConnectionInspector}

	for _, test := range tests {
		t.Run(test.scenario, func(t *testing.T) {
			updater.Update(&test.newValue, &test.oldValue)
