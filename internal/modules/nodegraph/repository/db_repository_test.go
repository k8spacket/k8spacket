...
var dbState = []model.ConnectionItem{
	{LastSeen: time.Now().Add(time.Hour * -1), Src: "test"},
	{LastSeen: time.Now(), SrcNamespace: "test", SrcName: "test"},
	{LastSeen: time.Now().Add(time.Hour), DstNamespace: "test", Dst: "test"},
	{LastSeen: time.Now().Add(time.Hour * 2), DstName: "test"},
	{LastSeen: time.Now().Add(time.Hour * 2)},
	{LastSeen: time.Now().Add(time.Hour * 1000)},
}

type mockDb struct {
	Db          db.Db[model.ConnectionItem]
	queryResult []model.ConnectionItem
}

func (mock *mockDb) Read(key string) (model.ConnectionItem, error) {
	if key == "error" {
...
func (mock *mockDb) Query(query *bolthold.Query) ([]model.ConnectionItem, error) {
...
func (mock *mockDb) QueryMatchFunc(field string, matchFunc func(*model.ConnectionItem) (bool, error)) bolthold.Query {
...
func (mock *mockDb) Upsert(key string, value *model.ConnectionItem) error {
	if key == "error" {
...
func TestRead(t *testing.T) {
	...
	repository := DbRepository{mockDBHandler}
