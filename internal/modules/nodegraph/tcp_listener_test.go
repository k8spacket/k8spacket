...
func (mockNodegraphService *mockNodegraphService) update(src string, srcName string, srcNamespace string, dst string, dstName string, dstNamespace string, persistent bool, bytesSent float64, bytesReceived float64, duration float64, closed bool) {
	mockNodegraphService.client = src
	mockNodegraphService.server = dst
}
