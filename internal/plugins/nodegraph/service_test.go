...
func (mockHandlerIO *mockHandlerIO) ReadFile(name string) ([]byte, error) {
	...
	// Return hardcoded fields.json content for testing
	return []byte(`{
  "edges_fields": [
    {
      "field_name": "id",
  ...
  "nodes_fields": [
    ...
    {
      "field_name": "mainStat",
      "displayName": "{{mainStatDisplayName}}",
      "type": "string"
    },
    {
      "field_name": "secondaryStat",
      "displayName": "{{secondaryStatDisplayName}}",
      "type": "string"
    },
    {
      "color": "{{arc1color}}",
      "field_name": "arc__1",
      "type": "number",
      "displayName": "{{arc1DisplayName}}"
    },
    {
      "color": "{{arc2color}}",
      "field_name": "arc__2",
      "type": "number",
      "displayName": "{{arc2DisplayName}}"
    }
  ]
}`), nil
}
...
func TestGetConnections(t *testing.T) {
	...
	service := &Service{mockRepository, &stats.Factory{}, &httpclient.HttpClient{}, &k8sclient.K8SClient{}, &handlerio.HandlerIO{}, sync.Mutex{}}
...
func TestUpdate(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.item.Src, func(t *testing.T) {
			...
			service.recordConnection(ConnectionUpdate{
				Src:           "src",
				SrcName:       "srcName",
				SrcNamespace:  "srcNs",
				Dst:           "dst",
...
func TestBuildO11yResponse(t *testing.T) {
	...
	var tests = []struct {
		...
		{"error", &model.NodeGraph{}, "[api] Cannot collect stats"},
		{"read", &model.NodeGraph{}, "failed reading"},
		{"parse", &model.NodeGraph{}, "invalid character 'p'"},
	}

	mockRepository := &mockRepository{}
	service := &Service{mockRepository, &stats.Factory{}, &mockHttpClient{}, &mockK8SClient{}, &handlerio.HandlerIO{}, sync.Mutex{}}
...
func TestGetO11yStatsConfig(t *testing.T) {
	...
	var tests = []struct {
		...
		{"connection", Fields{
			...
			NodesFields: []Field{
				...
				{FieldName: "arc__2", Type: "number", Color: "red", DisplayName: "Short-lived connections"}}}, ""},
		{"bytes", Fields{
			EdgesFields: []Field{
				{FieldName: "id", Type: "string", Color: "", DisplayName: ""},
				{FieldName: "source", Type: "string", Color: "", DisplayName: ""},
			...
			NodesFields: []Field{
				...
				{FieldName: "arc__2", Type: "number", Color: "yellow", DisplayName: "Bytes responded"}}}, ""},
		{"duration", Fields{
			EdgesFields: []Field{
				{FieldName: "id", Type: "string", Color: "", DisplayName: ""},
				{FieldName: "source", Type: "string", Color: "", DisplayName: ""},
			...
			NodesFields: []Field{
				...
				{FieldName: "arc__2", Type: "number", Color: "white", DisplayName: "Max duration"}}}, ""},
	...
	for _, test := range tests {
		t.Run(test.scenario, func(t *testing.T) {
			...
			service := &Service{mockRepository, &stats.Factory{}, &mockHttpClient{}, &mockK8SClient{}, mockHandlerIO, sync.Mutex{}}
